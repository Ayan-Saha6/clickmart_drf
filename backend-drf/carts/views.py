...
class AddToCartView(APIView):
    ...
    def post(self,request):
        ...
        if not product_id:
            return Response({'errors': 'product_id is required'})
        ...
        # get or create the cartitem ,means if the cartitem does not exist with that particular item then we create the cartitem,if that product already exist we will onnly increase the quantity
...
class ManageCartItemView(APIView):
    ...
    def patch(self,request,item_id):
        #validate the payload(incoming data)
